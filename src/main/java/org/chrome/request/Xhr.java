...
public class Xhr {
	...
	public static List<Xhr> all(Protocol client) throws IOException, InterruptedException {
		Request network = new Request(ENABLE_NETWORK);
		Integer id = network.getId();
		client.send(network);
		Request reload = new Request(RELOAD_PAGE);
	...
	private static List<Xhr> xhr(Protocol client, Integer id) throws IOException, InterruptedException {
		...
		while (true) {
			...
			if (REQUEST_WILLBESENT.equals(rs.value(mKey)) && XHR.equals(rs.value(tKey)) && XHR.equals(rs.value(xKey))) {
	...
	private final static String RELOAD_PAGE = "Page.reload";

	private final static String ENABLE_NETWORK = "Network.enable";
